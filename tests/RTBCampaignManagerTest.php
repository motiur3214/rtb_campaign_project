...
require_once dirname(__DIR__) . '/application/RTBCampaignManager.php';

use PHPUnit\Framework\TestCase;

class RTBCampaignManagerTest extends TestCase
{
//    valid request
    public function testHandleBidRequestWithValidRequest()
    {
        $bidRequest = file_get_contents(__DIR__ .'/example_bid_request.json');
        if ($bidRequest === false) {
            die('Error: Unable to load the file.');
        }
        $ex_campaign_arr = [
            [
        ...
        $campaignManager = new RTBCampaignManager($bidRequest, $ex_campaign_arr);
        $response = $campaignManager->handleBidRequest();
    ...
    public function testHandleBidRequestWithInValidRequest()
    {
        $bidRequest = file_get_contents('example_request_body.json');
        $campaigns = [
            [
                "campaignname" => "Test_Banner_13th-31st_march_Developer",
                "advertiser" => "TestGP",
        ...
        $campaignManager = new RTBCampaignManager($bidRequest, $campaigns);
        $response = $campaignManager->handleBidRequest();
